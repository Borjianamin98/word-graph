...
ANCHORS_TOPIC_NAME=anchors
KEYWORDS_TOPIC_NAME=keywords
...
./run-in-container.sh kafka kafka-topic-init.sh "${ANCHORS_TOPIC_NAME}"
./run-in-container.sh kafka kafka-topic-init.sh "${KEYWORDS_TOPIC_NAME}"
