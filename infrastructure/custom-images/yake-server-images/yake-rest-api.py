...
def log_request_info():
    json_body = str(request.json)
    app.logger.info('Body: %s', (json_body[:120] + '...') if len(json_body) > 120 else json_body)
...
def handle_yake():
    ...
    try:
        ...
        return jsonify({"error", f"Unable to create yake instance: {e}"}), HTTPStatus.BAD_REQUEST
    ...
    try:
        ...
        return jsonify({"error": "Language not supported"}), HTTPStatus.BAD_REQUEST
    except Exception as e:
        return jsonify({"error": f"Unexpected error: {e}"}), HTTPStatus.BAD_REQUEST
